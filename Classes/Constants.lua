...
function PDTHHudCoreConstants:init(parent)
	...
	local scale_bar = parent.Options:GetValue("HUD/ScaleBar")
    local OGTMHealth = parent.Options:GetValue("HUD/OGTMHealth")
    local OGScale = parent.Options:GetValue("HUD/OGScale")
    ...
    self.main_character_font_size = (15 + (OGScale and 2 or 0)) * scale
    self.main_character_y_offset = 2

    self.main_equipment_size = (36 + (OGScale and 11 or 0)) * scale
    self.main_equipment_font_size = (18 + (OGScale and 4 or 0)) * scale

    self.main_equipment_y_offset_multiplier = 1.75

    self.main_bag_gap = 5 * scale

    self.main_firemode_font_size = (14 + (OGScale and 2 or 0)) * scale
    self.main_ammo_font_size = (18 + (OGScale and 10 or 0)) * scale
    ...
    self.present_h = (40 + (OGScale and 27 or 0)) * scale
    self.present_title_fsize = (20 + (OGScale and 12 or 0)) * scale
    self.present_text_fsize = (20 + (OGScale and 10 or 0)) * scale
    self.present_text_gap = (1 + (OGScale and 2 or 0)) * scale
    self.present_title_y_offset = 1 * scale

	self.interact_bar_size = 36 * scale_bar

    self.interact_scale_w = 15
    self.interact_scale_h = 1

    self.interact_border = 0 * scale_bar
    self.interact_text_x_offset = 4 * scale_bar

    self.interact_bitmap_x_offset = 2 * scale_bar

    self.interact_h_multiplier = 1.8

    self.interact_font_size = 15 * scale_bar
    self.interact_invalid_font_size = 27 * scale_bar

    self.hostages_font_size = (16 + (OGScale and 8 or 0)) * scale

    self.assault_h = (75 + (OGScale and 22 or 0)) * scale
    self.assault_w = (85 + (OGScale and 24 or 0)) * scale
    self.assault_font_size = (22 + (OGScale and 6 or 0)) * scale
    self.assault_y_offset = 5 * scale

    self.no_return_t_font_size = 19 * scale
    self.no_return_timer_font_size = (24 + (OGScale and 13 or 0)) * scale
    self.no_return_timer_font_size_pulse = 32 * scale

    self.casing_size = 80 * scale
    self.casing_y_offset = (OGScale and -10 or 0) * scale
    ...
    self.tm_condition_font_size = (23 + (OGTMHealth and 5 or 0)) * scale
    self.tm_condition_font_size_flash = (38 + (OGTMHealth and 5 or 0)) * scale

    self.tm_equipment_inflation = 5

    self.tm_equipment_font_size = 16 * scale

    self.tm_name_font_size = (14 + (OGScale and 3 or 0)) * scale
    ...
    self.bag_text_size = (16 + (OGScale and 2 or 0)) * scale
    self.bag_size_h = 128 * scale
    self.bag_size_w = 256 * scale

    self.hint_font_size = (24 + (OGScale and 1 or 0)) * scale
end
