...
function HUDManager:set_teammate_ammo_amount(id, selection_index, max_clip, current_clip, current_left, max, force)
	...
	if id == HUDManager.PLAYER_PANEL and PDTHHud.Options:GetValue("HUD/spooky_ammo") then
...
function HUDManager:set_teammate_custom_radial(i, data)
	...
	if PDTHHud.Options:GetValue("HUD/Swansong") and i == HUDManager.PLAYER_PANEL then
...
--------------------------------------
-- This is only used from player states
function HUDManager:show_progress_timer_bar(current, total, icon_id)
	if not icon_id then
		if managers.player._current_state == 'mask_off' then
			icon_id = 'terrorist_head' -- masking up
		elseif managers.player._current_state == 'driving' then
			icon_id = 'equipment_ejection_seat'  --leaving vehicle
		elseif managers.player._current_state == 'standard' or managers.player._current_state == 'carry' then
			local eq = managers.player:selected_equipment()
			if eq and eq.equipment then
				icon_id = 'equipment_'..eq.equipment -- deployable
				if icon_id == 'equipment_sentry_gun' then icon_id = 'equipment_sentry' end 
				if icon_id == 'equipment_sentry_gun_silent' then icon_id = 'equipment_sentry_silent' end --it's complete shit but gets the job done
			end
		end
	end
	--icon_id = 'equipment_drill'
	self._progress_timer:show_interaction_bar(current, total, icon_id)
end
