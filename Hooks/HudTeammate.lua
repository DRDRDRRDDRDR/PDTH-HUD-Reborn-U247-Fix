if PDTHHud.Options:GetValue("HUD/MainHud") and not (restoration and restoration:all_enabled("HUD/MainHUD", "HUD/Teammate")) then
    function HUDTeammate:init(i, teammates_panel, is_player, height)
        ...
        self.health_w = self.health_h * ((not is_player and PDTHHud.Options:GetValue("HUD/OGTMHealth")) and const.og_health_ratio or const.health_ratio)
        ...
        if not main_player then
            ...
            character_icon = self._player_panel:bitmap({
                ...
                w = self.health_h,
                h = self.health_h,
                visible = PDTHHud.Options:GetValue("HUD/OGTMHealth"),
                layer = 1
            })
        ...
        if main_player or not PDTHHud.Options:GetValue("HUD/OGTMHealth") then
        ...
        local name = teammate_panel:text({
            ...
            y = 0,
            vertical = "bottom",
            font_size = const.tm_name_font_size,
        ...
        if main_player then
            ...
            name:set_left((PDTHHud.Options:GetValue("HUD/OGTMHealth") and character_icon:right() or radial_health_panel:right() ) + const.tm_name_gap)
		...
		local radial_delayed_damage_health = radial_delayed_damage_panel:bitmap({
			texture = "guis/textures/pdth_hud/hud_dot",
        ...
        condition_icon:set_center_x((main_player or not PDTHHud.Options:GetValue("HUD/OGTMHealth")) and radial_health_panel:center_x() or character_icon:center_x())
        condition_icon:set_center_y((main_player or not PDTHHud.Options:GetValue("HUD/OGTMHealth")) and radial_health_panel:center_y() or character_icon:center_y())
		...
		if PDTHHud.Options:GetValue("HUD/Melee") then	
			self:add_special_equipment({
				id = "melee_weapon",
				icon = "",
				amount = 1,
        ...
        local interact_panel = self._player_panel:panel({
            ...
            visible = false,
            layer = 14,
            w = (main_player or not PDTHHud.Options:GetValue("HUD/OGTMHealth")) and radial_health_panel:h() or character_icon:h(),
            h = (main_player or not PDTHHud.Options:GetValue("HUD/OGTMHealth")) and radial_health_panel:h() or character_icon:h()
        })

        interact_panel:set_center_x((main_player or not PDTHHud.Options:GetValue("HUD/OGTMHealth")) and radial_health_panel:center_x() or character_icon:center_x())
        interact_panel:set_center_y((main_player or not PDTHHud.Options:GetValue("HUD/OGTMHealth")) and radial_health_panel:center_y() or character_icon:center_y())
		...
		has_no_armor = managers.player:has_category_upgrade("player", "armor_to_health_conversion")
        self.armor_amount = has_no_armor and 0 or 1
        ...
        local name_revives = teammate_panel:child("name")
    ...
    function HUDTeammate:_create_weapon_firemode(weap_details, i)
		...
		locked_to_auto = managers.weapon_factory:has_perk("fire_mode_burst", weap_details.factory_id, weap_details.blueprint)
        ...
        elseif self._firemode_secondary_mapping then
    ...
    function HUDTeammate:set_ammo_amount_by_type(type, max_clip, current_clip, current_left, max, force)
        local const = PDTHHud.constants
        local scale = PDTHHud.Options:GetValue("HUD/Scale")
        local ammoAmount = self._main_player and PDTHHud.Options:GetValue("HUD/spooky_ammo") and current_left + current_clip or current_left
    ...
    function HUDTeammate:set_health(data)
		...
		self:update_delayed_damage()
        --local color = amount < 0.33 and PDTHHud.constants.health_main_colour_damaged or PDTHHud.constants.health_main_colour
    ...
    function HUDTeammate:set_armor(data)
        ...
        local amount = data.total ~= 0 and data.current / data.total or 0
    ...
    function HUDTeammate:set_name(teammate_name)
        ...
        if PDTHHud.Options:GetValue("HUD/UppercaseTeam") then
    ...
    function HUDTeammate:set_state(state)
        ...
        if not self._main_player then
            ...
            if is_player then
                ...
                radial_health_panel:set_visible(not PDTHHud.Options:GetValue("HUD/OGTMHealth"))
    ...
    function HUDTeammate:check_grenade(data)
        ...
        self.grenades_amount = data.amount
    ...
    function HUDTeammate:check_grenades_amounts(data)
        return self.grenades_amount
    end
    ...
    function HUDTeammate:set_grenades(data)
		...
		if alive(grenades_panel) then
            self.grenades_cooldown_pocket = grenades_panel:text({
                name = "grenades_cooldown_pocket",
                font_size = 17,
                vertical = "top",
                align = "right",
                font = tweak_data.menu.small_font,
            ...
            self.grenade_buff_pocket = grenades_panel:text({
                name = "grenade_buff_pocket",
                font_size = 17,
                vertical = "bottom",
                align = "right",
                font = tweak_data.menu.small_font,
            ...
            self.grenade_buff = grenades_panel:text({
                name = "grenade_buff",
                font_size = 17,
                vertical = "bottom",
                align = "left",
            ...
            self.grenades_cooldown_text = grenades_panel:text({
                name = "grenades_cooldown_text",
                font_size = 17,
                vertical = "top",
                align = "left",
                font = tweak_data.menu.small_font,
    ...
    function HUDTeammate:grenade_id(grenade)
		if self._main_player then
    ...
    function HUDTeammate:set_grenade_cooldown(data)
        ...
        -- local grenades_panel = self._player_panel:child("grenades_panel")
        -- self.grenades_cooldown_text = grenades_panel:child("grenades_cooldown_text")

        if not end_time or not duration then
            self.grenades_cooldown_pocket:stop()
            self.grenades_cooldown_pocket:set_visible(false)

            return
        end

        if alive(self.grenades_cooldown_pocket)  and self:grenade_id("pocket_ecm_jammer") then
            self.grenades_cooldown_pocket:set_visible(true)
            self.grenades_cooldown_pocket:stop()
            self.grenades_cooldown_pocket:animate(function(o)
                local time_left = end_time - managers.game_play_central:get_heist_timer()
                local left_time = time_left / duration
                local cooldown_time = duration * left_time
                over(time_left, function(p)
                    self.grenades_cooldown_pocket:set_text(1 + math.floor(cooldown_time * (1-p)))
                end)
                self.grenades_cooldown_pocket:set_visible(false)
            end)
        end

        if alive(self.grenade_buff_pocket) and self:grenade_id("pocket_ecm_jammer") then
            self.grenade_buff_pocket:set_visible(true)
            self.grenade_buff_pocket:animate(function(o)
                local max_duration = tweak_data.upgrades.values.player.pocket_ecm_jammer_base[1].duration
				local t = Application:time()
				local expire_t = t + max_duration
                over(max_duration, function(p)
                    self.grenade_buff_pocket:set_text(1 + math.floor(max_duration * (1-p)))
                end)
                self.grenade_buff_pocket:set_visible(false)
            end)
        end

        if alive(self.grenade_buff) and not self:grenade_id("pocket_ecm_jammer") then
            self.grenade_buff:set_visible(true)
            self.grenade_buff:animate(function(o)
                local max_duration = tweak_data.blackmarket.projectiles[managers.blackmarket:equipped_grenade()].duration
				local t = Application:time()
				local expire_t = t + max_duration
                over(max_duration, function(p)
                    self.grenade_buff:set_text(1 + math.floor(max_duration * (1-p)))
                end)
                self.grenade_buff:set_visible(false)
            end)
        end

        if alive(self.grenades_cooldown_text) and not self:grenade_id("pocket_ecm_jammer") then
            self.grenades_cooldown_text:set_visible(true)
            self.grenades_cooldown_text:animate(function(o)
                local time_left = end_time - managers.game_play_central:get_heist_timer()
                local left_time = time_left / duration
                local cooldown_time = duration * left_time
                over(time_left, function(p)
                    self.grenades_cooldown_text:set_text(1 + math.floor(cooldown_time * (1-p)))
                end)
                self.grenades_cooldown_text:set_visible(false)
            end)
        end
    ...
    function HUDTeammate:add_special_equipment(data)
        ...
        if not self._main_player then
            teammate_height = self._player_panel:child("radial_health_panel"):h() - name:h() + (const.tm_equipment_inflation * PDTHHud.Options:GetValue("HUD/Scale"))
        end
    ...
    function HUDTeammate:set_special_equipment_amount(equipment_id, amount, ignore_checks)
        ...
        local function apply_setting(i, special)
			...
			local grayAmount = panel:child("bitmap")
			local color = amount == 0 and Color(0.5, 1, 1, 1) or Color.white
			txtAmount:set_color(color)
			grayAmount:set_color(color)
            ...
            if not special.weapon and type(amount) == "number" then
                ...
                if amount < 1 and not ignore_checks and special.show_grenades_at_zero then
    ...
    function HUDTeammate:set_special_equipment_image(equipment_id, ...)
        ...
        local success, err = pcall(function(...)
            ...
            if panel and panel:name() == equipment_id then
                ...
                if special.weapon then
                    image, texture_rect = PDTHHud.textures:get_weapon_texture(...)
                else
        ...
        for i, special in ipairs(self._weapons) do
            ...
            if panel and panel:name() == equipment_id then
                special.icon_data = {...}
                local image, texture_rect = PDTHHud.textures:get_weapon_texture(...)
                panel:child("bitmap"):set_image(image, unpack(texture_rect))
                return
            end
        end
    ...
    function HUDTeammate:layout_special_equipments()
        ...
        for i, special in ipairs(special_equipment) do
            ...
            elseif panel then
                panel:set_left(special_equipment[i - 1] and special_equipment[i - 1].panel and (special_equipment[i - 1].panel:right() + gap) or (PDTHHud.Options:GetValue("HUD/OGTMHealth") and character_icon:right() or radial_health_panel:right()) + gap)
    ...
    function HUDTeammate:refresh_special_equipment()
        ...
        for i, special in ipairs(self._weapons) do
            local panel = special.panel
            local image, texture_rect = PDTHHud.textures:get_weapon_texture(unpack(special.icon_data))
            panel:child("bitmap"):set_image(image, unpack(texture_rect))
        end
    end
    ...
    function HUDTeammate:teammate_progress(enabled, tweak_data_id, timer, success)
        local character_icon = self._player_panel:child("character_icon")
        if character_icon and PDTHHud.Options:GetValue("HUD/OGTMHealth") then
        ...
        elseif success then
            ...
            local circle = CircleBitmapGuiObject:new(panel, {
                rotation = 360,
                radius = radius * PDTHHud.Options:GetValue("HUD/Scale"),
    ...
    function HUDTeammate:_animate_timer()
        ...
        while self._timer >= 0 do
            ...
            if self._timer_paused == 0 then
                ...
                if PDTHHud.Options:GetValue("HUD/OGTMHealth") then
    ...
    function HUDTeammate:RefreshPortraits()
        ...
        if not self._ai or not PDTHHud.Options:GetValue("HUD/OGTMHealth") then
            local texture, rect = PDTHHud.textures:get_portrait_texture(self._character, "health", self._main_player)
            if texture then
                local y_offset = rect[4] * (1 - self.health_amount)
                local h_offset = self.health_h * (1 - self.health_amount)
				has_no_armor = managers.player:has_category_upgrade("player", "armor_to_health_conversion")
                radial_health:set_color(self._ai and Color.white or PDTHHud.utils:GetPortraitColour(self.health_amount))
            ...
            texture, rect = PDTHHud.textures:get_portrait_texture(self._character, "armor", self._main_player)
            if texture then
        ...
        texture, rect = PDTHHud.textures:get_portrait_texture(self._character, "bg", self._main_player)
        if texture then
            radial_bg:set_image(texture, unpack(rect))
        end

        if PDTHHud.Options:GetValue("HUD/OGTMHealth") and not self._main_player then
            local texture, rect = PDTHHud.textures:get_portrait_texture(self._character, "tm")
            if texture then
                character_icon:set_image(texture, unpack(rect))
            end
        end

        radial_health_panel:set_visible(not self._ai or not PDTHHud.Options:GetValue("HUD/OGTMHealth"))
    ...
    function HUDTeammate:_animate_update_absorb(o, radial_absorb_shield_name, radial_absorb_health_name, var_name, blink)
        ...
        local textureA, rectA = PDTHHud.textures:get_portrait_texture(self._character, "armor", self._main_player)
    ...
    function HUDTeammate:set_ability_radial(data)
        ...
        local y_offset = 130 * (1 - progress)
        local h_offset = self.health_h * (1 - progress)
		ability_meter:set_texture_rect(0, y_offset, 64, 130 - y_offset)
        radial_ability_panel:set_h(self.health_h - h_offset)
        radial_ability_panel:set_bottom(radial_bg:bottom())
    ...
    function HUDTeammate:activate_ability_radial(time_left, time_total)
		...
		local function anim(o)
			...
			over(time_left, function (p)
				...
				local h_offset = self.health_h * (1 - progress)
				ability_meter:set_texture_rect(0, y_offset, 64, 130 - y_offset)
				ability_meter:set_h(self.health_h - h_offset)
				ability_meter:set_bottom(radial_bg:bottom())
				ability_meter:set_color(Color(1, progress, 1, 1))
			end)
    ...
    function HUDTeammate:set_revives_amount(revive_amount)
        self._revive_amount = revive_amount
        local const = PDTHHud.constants
        if revive_amount then
            ...
            if revive_amount_text then
                if self._main_player then
                    local character_name = string.upper(managers.localization:text("menu_" .. self._character))
                    character_text:set_text(tostring(math.max(revive_amount - 1, 0)) .. "x" .. "\n" .. character_name)
                    managers.hud:make_fine_text(character_text)
                    character_text:set_center_x(radial_health_panel:center_x())
                    character_text:set_bottom(radial_health_panel:h() - const.main_character_y_offset)
                else
...
if restoration and restoration.Options:GetValue("OTHER/ResModChallenges") then
        function HUDTeammate:update_delayed_damage()
            ...
            local health_ratio = health_radial:h() / self.health_h
            ...
            delayed_damage_health:set_h(self.health_h * health_damage_ratio)
            delayed_damage_health:set_top(health_radial:top())
            delayed_damage_health:set_texture_rect(0,((1- health_ratio) * 64),130,130 * health_damage_ratio)
        end
    else
        function HUDTeammate:update_delayed_damage()
            local damage = self._delayed_damage or 0
            ...
            local armor_ratio = armor_radial:h() / self.health_h
            local health_max = self._health_data.total
            local health_current = self._health_data.current
            local health_ratio = health_radial:h() / self.health_h
            ...
            delayed_damage_health:set_visible(health_damage_ratio > 0)
            delayed_damage_armor:set_h(self.health_h * armor_damage_ratio)
            delayed_damage_armor:set_top(armor_radial:top())
            delayed_damage_armor:set_texture_rect(0,((1- armor_ratio) * 64),130,130 * armor_damage_ratio)
            delayed_damage_health:set_h(self.health_h * health_damage_ratio)
            delayed_damage_health:set_top(health_radial:top())
            delayed_damage_health:set_texture_rect(0,((1- health_ratio) * 64),130,130 * health_damage_ratio)
        end
