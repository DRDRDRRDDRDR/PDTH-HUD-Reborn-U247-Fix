...
PortraitPreviewGUI.main_y_offset = 120
...
function PortraitPreviewGUI:create_portrait(o, char_tbl)
    ...
    o:bitmap({
        name = "health",
        texture = "",
        color = PDTHHud.constants.health_main_colour,
        blend_mode = "normal",
        layer = 1
    })
...
function PortraitPreviewGUI:refresh_portrait_size(o)
    ...
    text:set_h(h)
    text:set_bottom(o:h() - PDTHHud.constants.main_character_y_offset)
end
...
function PortraitPreviewGUI:display_portrait(o, char_tbl, use_slider_offsets)
    ...
    local texture, rect = PDTHHud.textures:get_portrait_texture(char_tbl.name, "health", true)
    ...
    if use_slider_offsets and self._health_slider and self._focused_portrait_panel then
        ...
        health:set_color(PDTHHud.utils:GetPortraitColour(1 - value))

        y_offset = rect[4] * value
        h_offset = bg:h() * value
    else
        health:set_color(PDTHHud.utils:GetPortraitColour())
    end
    ...
    texture, rect = PDTHHud.textures:get_portrait_texture(char_tbl.name, "armor", true)
    ...
    texture, rect = PDTHHud.textures:get_portrait_texture(char_tbl.name, "bg", true)
...
function PortraitPreviewGUI:create_portrait_options()
    ...
    local portrait_opt = PDTHHud.Options:GetOption("HUD/portraits/" .. tweak_data.criminals.characters[self._focused_portrait_i].name)
...
function PortraitPreviewGUI:mouse_pressed(button, x, y)
    ...
    if self._portrait_options_panel and self._portrait_options_panel:visible() and button == Idstring("0") then
        ...
        if self._portrait_options then
            ...
            local portrait_opt = PDTHHud.Options:GetOption(opt_path)
            ...
            for i, opt in pairs(self._portrait_options) do
                ...
                if opt:inside(x, y) then
                    ...
                    PDTHHud.Options:SetValue(opt_path, i)
                    self:display_portrait(self._focused_portrait_panel, tweak_data.criminals.characters[self._focused_portrait_i], true)
