if PDTHHud.Options:GetValue("HUD/Objectives") and not (restoration and restoration:all_enabled("HUD/MainHUD", "HUD/Presenter")) then
    function HUDPresenter:init(hud)
        self._active_notifications = 0
        local const = PDTHHud.constants
        self._hud_panel = managers.hud._fullscreen_workspace:panel():gui(PlayerBase.PLAYER_INFO_HUD_PD2, {})
    ...
    function HUDPresenter:_do_it(queued)
        if self._active_notifications > 3 then
